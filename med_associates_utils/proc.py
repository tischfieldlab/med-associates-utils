...
def calculate_event_series_stats(sessions: SessionCollection, event: Union[str, list[str]], max_events: int, max_time: int, include_meta: FieldList = 'all',
                                 time_key: str = 'time', event_key: str = 'event', form: Literal['long', 'wide'] = 'wide') -> pd.DataFrame:
            ...
            if df.empty:
                ...
                summaries.append({
                    **meta,
                    ...
                    'finished': max_time if count < max_events else last,  # if the trial was completed, use last event time, otherwise use the max trial duration
                    'count': count,
    ...
    if form == 'long':
        ...
        id_vars = list(set(summaries.columns.to_list()) - set(value_vars))
        return summaries.melt(id_vars=id_vars, value_vars=value_vars, var_name='metric', value_name='value', ignore_index=True)
    elif form == 'wide':
        return summaries
    else:
